...
(function (global) {
  ...
  const CONFIG = {
    ...
    po_fill: {
      supabase: {
        ...
        columns: [
          ...
          { label: 'OrderedRec',  field: 'ordered_recorded', type: 'number' },
    ...
    lettuce_harvest: {
      supabase: {
        ...
        select: 'harvest_date,pond,side,seed_name,boards_per_pond,pounds_per_board,greenhouse_net_weight,max_lb,harvest_disposition,harvest_disposition_reason',
        ...
        columns: [
          ...
          // Dumped / donated batches: real boards, zero pounds, on purpose. Kept out
          // of the lb/board average and shown on their own line.
          { label: 'Disposition',         field: 'harvest_disposition',        type: 'string' },
          { label: 'DispositionReason',   field: 'harvest_disposition_reason', type: 'string' },
        ],
      },
    },
    ...
    lettuce_schedule: {
      supabase: {
        ...
        select: 'pond,side,variety,seedname,seedsperboard,boards,seeding_date,pond_date,estimated_harvest_date,harvest_date,id,harvest_disposition,harvest_disposition_reason',
        columns: [
          // Batch id: the Issues tab addresses a single cycle by it when it
          // resolves a past-due row (lettuce_resolve_late_cycle).
          { label: 'BatchId',        field: 'id',                     type: 'string' },
          { label: 'Pond',           field: 'pond',                   type: 'string' },
          { label: 'Side',           field: 'side',                   type: 'string' },
          { label: 'Variety',        field: 'variety',                type: 'string' },
          ...
          { label: 'HarvestDate',    field: 'harvest_date',           type: 'date'   },
          // NULL = harvested and sold as normal. 'Dumped' / 'Donation' = the boards
          // came off the pond on harvest_date but produced nothing sellable, so the
          // weights are zero on purpose and yield figures must not average them in.
          { label: 'Disposition',       field: 'harvest_disposition',        type: 'string' },
          { label: 'DispositionReason', field: 'harvest_disposition_reason', type: 'string' },
        ],
      },
  ...
  async function fetchTable(logicalName, opts = {}) {
    ...
    const client = await getSupabaseClient('prod');
    const pageSize = 1000;
    const all = [];
