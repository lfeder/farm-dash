...
(function (global) {
  ...
  const CONFIG = {
    ...
    // Sales-page Budget tab: per-farm weekly/monthly budget targets (Bdg line),
    // prod-only (fetch with {mode:'prod'}). Views unpivot finance_sales_budget_*
    // × prices/pounds-per-case; dollars are already in $000. See finance_ tables.
    budget_weekly_farm: {
      supabase: {
        table: 'finance_budget_weekly_farm_v',
        select: '*',
        orderBy: 'isoweek', // no id column; <1000 rows so single page
        columns: [
    ...
    budget_monthly_farm: {
      supabase: {
        table: 'finance_budget_monthly_farm_v',
        select: '*',
        orderBy: 'month', // no id column; <1000 rows so single page
        columns: [
          { label: 'ISOYear', field: 'isoyear', type: 'number' },
          { label: 'Month',   field: 'month',   type: 'number' },
          { label: 'Farm',    field: 'farm',    type: 'string' },
