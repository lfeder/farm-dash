...
(function (global) {
  ...
  const CONFIG = {
    ...
    // On-grade harvest (the "Grown" line): pounds grown by iso-week / month /
    // farm / variety / grade. Cuke from grow_cuke_harvest, Lettuce from
    // grow_lettuce_pond_harvest (seed_name -> W/A/L).
    harvest_weekly: {
      supabase: {
        table: 'grow_harvest_weekly_v', select: '*', orderBy: 'isoyear',
        columns: [
          { label: 'isoyear', field: 'isoyear', type: 'number' },
          { label: 'isoweek', field: 'isoweek', type: 'number' },
          { label: 'year',    field: 'year',    type: 'number' },
