...
(function (global) {
  ...
  const CONFIG = {
    ...
    invoices_daily: {
      ...
      // Supabase returns raw rows (with the filter columns) and the page
      // aggregates client-side — same result, just not reduced server-side.
      supabase: {
        table: 'sales_invoice_v',
        select: 'invoice_date,farm_id,dollars,product_code,customer_group',
  ...
  const SOURCE_GROUPS = [
    ...
    { key: 'cuke',     label: 'Cuke grow',    tables: ['cuke_harvest'] },
    { key: 'lettuce',  label: 'Lettuce grow', tables: ['lettuce_harvest'] },
    { key: 'fsafe',    label: 'Food safety',  tables: ['fsafe_emp', 'fsafe_ca'], def: 'sheets' },
  ...
  function renderSourcePanel(button) {
    ...
    panel.style.cssText = [
      'position:absolute', 'z-index:1000', 'display:none', 'min-width:230px',
