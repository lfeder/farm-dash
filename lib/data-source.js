...
(function (global) {
  ...
  const CONFIG = {
    ...
    fsafe_emp: {
      ...
      supabase: {
        table: 'fsafe_result', select: '*', orderBy: 'sampled_at',
        columns: [
    ...
    // --- Payroll (prod-only; drives the WC / TDI wage-basis page) ---
    // Per-employee, per-check rows from the external payroll processor. The
    // WC/TDI page always fetches this with {mode:'prod'} since it's compliance
    // data. Default orderBy 'id' (uuid) keeps pagination stable across the
    // ~12k rows (check_date has ties that would skip/duplicate at page edges).
    hr_payroll: {
      sheets: [],
      supabase: {
        // Restricted view (wage-basis columns only) so the public anon key
        // can't reach net pay / taxes / child support on the base table.
        table: 'wc_tdi_payroll_v',
        select: 'id,check_date,pay_period_start,pay_period_end,wc,hr_employee_id,employee_name,hr_department_id,hr_work_authorization_id,overtime_pay,overtime_hours,hourly_rate,per_diem,auto_allowance,other_pay,gross_wage,payroll_processor,is_deleted',
        columns: [
          { label: 'CheckDate',   field: 'check_date',                type: 'date'    },
          { label: 'PeriodStart', field: 'pay_period_start',          type: 'date'    },
          { label: 'PeriodEnd',   field: 'pay_period_end',            type: 'date'    },
          { label: 'WC',          field: 'wc',                        type: 'string'  },
          { label: 'EmpId',       field: 'hr_employee_id',            type: 'string'  },
          { label: 'EmpName',     field: 'employee_name',             type: 'string'  },
          { label: 'Dept',        field: 'hr_department_id',          type: 'string'  },
          { label: 'WorkAuth',    field: 'hr_work_authorization_id',  type: 'string'  },
          { label: 'OtPay',       field: 'overtime_pay',              type: 'number'  },
          { label: 'OtHours',     field: 'overtime_hours',            type: 'number'  },
          { label: 'HourlyRate',  field: 'hourly_rate',               type: 'number'  },
          { label: 'PerDiem',     field: 'per_diem',                  type: 'number'  },
          { label: 'AutoAllow',   field: 'auto_allowance',            type: 'number'  },
          { label: 'OtherPay',    field: 'other_pay',                 type: 'number'  },
          { label: 'GrossWage',   field: 'gross_wage',                type: 'number'  },
          { label: 'Processor',   field: 'payroll_processor',         type: 'string'  },
          { label: 'IsDeleted',   field: 'is_deleted',                type: 'boolean' },
        ],
      },
    },
   ...
   * sources with a sheets/dev/prod selector (plus "Global" to clear the
  ...
  function renderSourcePanel(button) {
    ...
    SOURCE_GROUPS.forEach(g => {
      ...
      [['global', `Default (${g.def || globalMode})`], ['sheets', 'sheets'], ['dev', 'dev'], ['prod', 'prod']]
        .forEach(([val, text]) => {
          const o = document.createElement('option');
