  ...
  const CONFIG = {
    ...
    cuke_harvest_daily: {
      ...
      supabase: {
        ...
        select: '*',
        // The sheets query was `where B=2026` (current year). Match it, but off
        // the clock rather than a hardcoded year, so this does not silently go
        // blank on 1 Jan.
        where: [{ op: 'gte', col: 'harvest_date', val: yearStart }],
    ...
    fsafe_emp: {
      ...
      supabase: {
        ...
        where: [{ op: 'eq', col: 'result_pass', val: false }],
    ...
    // Pre-op checklists. Moved sheets -> prod on 08/11/26 when Sheets was
    // switched off. fsafe_gh_preop_v emits the exact column LABELS the
    // data_check_rule rows name (Checked Date / Approved to Harvest /
    // Greenhouse(s)), so the rule engine in daily/index.html is untouched.
    fsafe_cuke_preop: {
      sheets: [],
      supabase: {
        table: 'fsafe_gh_preop_v',
        select: 'checked_date,approved_to_harvest,greenhouses',
        where: [
          { op: 'eq',  col: 'org_id',       val: 'hawaii_farming' },
          { op: 'eq',  col: 'farm_key',     val: 'cuke' },
          { op: 'gte', col: 'checked_date', val: () => daysAgo(60) },
        ],
        orderBy: 'checked_date', tiebreak: 'checked_date',
        // Labels must match data_check_rule.preop_date_col / preop_flag_col /
        // preop_members_col exactly -- that is the contract with the rule engine.
        columns: [
          { label: 'Checked Date',        field: 'checked_date',        type: 'date'    },
          { label: 'Approved to Harvest', field: 'approved_to_harvest', type: 'string'  },
          // Greenhouse list (e.g. "bip_hi+bip_hk+jtl_01") drives the expected
          // GH count for the cuke check.
    ...
    fsafe_lettuce_preop: {
      ...
      supabase: {
        table: 'fsafe_gh_preop_v',
        select: 'checked_date,approved_to_harvest',
        where: [
          { op: 'eq',  col: 'org_id',       val: 'hawaii_farming' },
          { op: 'eq',  col: 'farm_key',     val: 'lettuce' },
          { op: 'gte', col: 'checked_date', val: () => daysAgo(60) },
        ],
        orderBy: 'checked_date', tiebreak: 'checked_date',
        columns: [
    ...
    // All 2026 cycles (filtered by date only, regardless of cycle status).
    lettuce_schedule: {
      supabase: {
